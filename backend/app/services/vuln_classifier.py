...
_FUZZY_RULES = [
    # SQL 注入
    ((r"\bsql\b", r"\bsqli\b", r"\bsql[ _-]?injection\b"), VulnerabilityType.SQL_INJECTION),
    # XSS
    ((r"\bxss\b", r"\bcross[ _-]?site\b"), VulnerabilityType.XSS),
    ...
    ((r"\bcommand[ _-]?injection\b", r"\bcode[ _-]?injection\b",
      ...
      r"\bshell[ _-]?injection\b", r"\bcmd[ _-]?injection\b"), VulnerabilityType.COMMAND_INJECTION),
    # 路径遍历（不用裸 "path"，因为匹配 empath）
    ((r"\bpath[ _-]?traversal\b", r"\bdirectory[ _-]?traversal\b",
      r"\blfi\b", r"\brfi\b"), VulnerabilityType.PATH_TRAVERSAL),
    # SSRF
    ((r"\bssrf\b", r"\bserver[ _-]?side[ _-]?request[ _-]?forgery\b"), VulnerabilityType.SSRF),
    # XXE
    ((r"\bxxe\b", r"\bxml[ _-]?external[ _-]?entity\b"), VulnerabilityType.XXE),
    # 认证绕过（不用裸 "auth"，因为匹配 author）
    ((r"\bauth[ _-]?bypass\b", r"\bauthentication[ _-]?bypass\b",
      r"\bauthz[ _-]?bypass\b"), VulnerabilityType.AUTH_BYPASS),
    # 硬编码密钥
    ((r"\bhardcoded\b", r"\bhard[ _-]?coded\b", r"\bsecret\b",
      r"\bpassword\b", r"\bcredential\b", r"\bapi[ _-]?key\b",
      r"\bprivate[ _-]?key\b"), VulnerabilityType.HARDCODED_SECRET),
    # 反序列化
    ((r"\bdeserializ", r"\bunserializ", r"\bpickle\b"), VulnerabilityType.DESERIALIZATION),
    # 弱加密
    ((r"\bweak[ _-]?crypto\b", r"\bweak[ _-]?hash\b", r"\bmd5\b", r"\bsha1\b"), VulnerabilityType.WEAK_CRYPTO),
    # 敏感数据暴露
    ((r"\bsensitive[ _-]?data\b", r"\bdata[ _-]?exposure\b"), VulnerabilityType.SENSITIVE_DATA_EXPOSURE),
    # IDOR
    ((r"\bidor\b",), VulnerabilityType.IDOR),
    # 竞态条件
    ((r"\brace[ _-]?condition\b",), VulnerabilityType.RACE_CONDITION),
    # 业务逻辑
    ((r"\bbusiness[ _-]?logic\b",), VulnerabilityType.BUSINESS_LOGIC),
    # 内存损坏
    ((r"\bmemory[ _-]?corruption\b", r"\bbuffer[ _-]?overflow\b"), VulnerabilityType.MEMORY_CORRUPTION),
]
